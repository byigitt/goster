...
function createStore(listenersRef, stateRef, onValueChange) {
  const store = {
    subscribe: (cb) => {
...
function useStoreSelector(selector) {
  ...
  const getSnapshot = React.useCallback(() => selector(storeContext.getState()), [storeContext, selector]);

  return React.useSyncExternalStore(storeContext.subscribe, getSnapshot, getSnapshot);
}
...
function MediaPlayerRootImpl(props) {
  const {
    onPlay,
    onPause,
    onEnded,
...
function MediaPlayerVolumeIndicator(props) {
  ...
  const volumeIndicatorVisible = useStoreSelector((state) => state.volumeIndicatorVisible);
      ...
      <MediaPlayerTooltip tooltip="Playback speed" shortcut={["<", ">"]}>
        <DropdownMenuTrigger asChild>
          <Button
            ...
            variant="ghost"
            size="icon"
            className={cn("h-8 w-16 aria-[expanded=true]:bg-accent/50", className)}>
            {mediaPlaybackRate}x
          </Button>
        </DropdownMenuTrigger>
      </MediaPlayerTooltip>
      ...
      <MediaPlayerTooltip tooltip="Settings">
        <DropdownMenuTrigger asChild>
          <Button
            ...
            variant="ghost"
            size="icon"
            className={cn("size-8 aria-[expanded=true]:bg-accent/50", className)}>
            <SettingsIcon />
          </Button>
        </DropdownMenuTrigger>
      </MediaPlayerTooltip>
